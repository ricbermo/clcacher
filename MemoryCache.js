...
MemoryCache.set = async (key, value, expires = DEFAULT_EXPIRES) => {
  ...
  try {
    await AsyncStorage.setItem(
      k,
      b64EncodeUnicode(JSON.stringify(storageValue))
    );
    return null;
  } catch (err) {
    return { error: err };
...
MemoryCache.get = async (key) => {
  ...
  try {
    ...
    v = v
      ? JSON.parse(Platform.OS == "android" ? b64DecodeUnicode(v) : v)
      : null;
...
MemoryCache.multiSet = (values, expires = DEFAULT_EXPIRES) => {
  return new Promise((resolve, reject) => {
    ...
    for (let key in values) {
      MemoryCache.set(key, values[key], expires)
        .then(() => {
          if (++counter == length) {
            resolve(null);
          }
...
MemoryCache.getAllValues = () =>
  MemoryCache.getAllKeys().then((all) =>
    all.reduce((prev, next) => {
